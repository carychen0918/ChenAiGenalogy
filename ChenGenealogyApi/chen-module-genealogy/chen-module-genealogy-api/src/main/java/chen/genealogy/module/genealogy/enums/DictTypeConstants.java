package chen.genealogy.module.genealogy.enums;

public interface DictTypeConstants {

    String GENERATION_STATUS = "genealogy_generation_status";
    String SCHOLARSHIP_TYPE = "genealogy_scholarship_type";
    String SCHOLARSHIP_STATUS = "genealogy_scholarship_status";
    String DISBURSE_METHOD = "genealogy_disburse_method";
    String ACTIVITY_STATUS = "genealogy_activity_status";
    String FEED_TYPE = "genealogy_feed_type";
    String FEED_STATUS = "genealogy_feed_status";
    String GRADE = "genealogy_grade";
    String DEED_CATEGORY = "genealogy_deed_category";
    String ARCHIVE_STATUS = "genealogy_archive_status";
}
