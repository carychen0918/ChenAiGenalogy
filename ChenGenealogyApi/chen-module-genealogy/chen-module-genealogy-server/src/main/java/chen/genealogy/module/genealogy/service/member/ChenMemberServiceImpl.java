...
@Service
@Validated
public class ChenMemberServiceImpl implements ChenMemberService {
    ...
    public Long createMember(MemberSaveReqVO reqVO) {
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public void updateMember(MemberSaveReqVO reqVO) {
    ...
    public MemberImportRespVO importMemberList(List<MemberImportExcelVO> list, Boolean updateSupport) {
        ...
        for (MemberImportExcelVO excel : list) {
            ...
            try {
                ...
                String genError = assignImportGeneration(req, excel);
                if (genError != null) {
                    failures.add("第" + row + "行：" + genError);
                    continue;
                }
    ...
    private String assignImportGeneration(MemberSaveReqVO req, MemberImportExcelVO excel) {
        Integer generationNo = excel.getGenerationNo();
        String word = StrUtil.trim(excel.getGenerationWord());
        if (generationNo == null && StrUtil.isBlank(word)) {
            return null;
        }
        if (generationNo != null && StrUtil.isNotBlank(word)) {
            GenerationDO gen = generationMapper.selectByFamilyNoAndWord(DEFAULT_FAMILY_ID, generationNo, word);
            if (gen == null) {
                return "第" + generationNo + "世不存在「" + word + "」字辈";
            }
            req.setGenerationId(gen.getId());
            return null;
        }
        if (StrUtil.isNotBlank(word)) {
            List<GenerationDO> byWord = generationMapper.selectListByFamilyAndWord(DEFAULT_FAMILY_ID, word);
            if (byWord.isEmpty()) {
                return "字辈「" + word + "」不存在";
            }
            if (byWord.size() > 1) {
                return "字辈「" + word + "」对应多个世代，请同时填写「世代」列";
            }
            req.setGenerationId(byWord.get(0).getId());
            return null;
        }
        List<GenerationDO> byNo = generationMapper.selectListByFamilyAndNo(DEFAULT_FAMILY_ID, generationNo);
        if (byNo.isEmpty()) {
            return "第" + generationNo + "世尚未配置字辈";
        }
        if (byNo.size() > 1) {
            return "第" + generationNo + "世存在多个字辈，请填写「字辈」列";
        }
        req.setGenerationId(byNo.get(0).getId());
        return null;
    }
