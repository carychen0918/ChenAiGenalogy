  ...
  <div class="hud">
    ...
    <header class="hero">
      ...
      <div class="hero-right">
        ...
        <el-button class="portal-btn" @click="router.push('/portal/home')">进入族人端门户</el-button>
      </div>
    </header>

    <div class="alerts">
      <button v-for="a in alerts" :key="a.label" class="pill" :class="{ hot: a.value > 0 }" @click="router.push(a.to)">
    ...
    <div class="panels">
      ...
      <section class="panel">
        ...
        <div class="ritual" v-if="summary.upcomingActivity" @click="router.push('/genealogy/worship/activity')">
          ...
          <div class="todo-time">
            ...
            · {{ summary.upcomingActivity.place }}
            · 已报 {{ summary.upcomingActivity.joinedCount || 0 }}/{{ summary.upcomingActivity.maxCount || '-' }}
          </div>
        </div>
      </section>
    </div>
...
const alerts = computed(() => [
  ...
  { label: '待发放', value: n(summary.value.pendingDisburse), to: '/genealogy/scholarship/disburse' },
  { label: '待补材料', value: n(summary.value.pendingMaterial), to: '/genealogy/scholarship/audit' }
])
const kpis = computed(() => [
  { en: 'MEMBERS', label: '成员总数', value: n(summary.value.memberCount), to: '/genealogy/family/member' },
  { en: 'ALIVE', label: '在世', value: n(summary.value.aliveCount), to: '/genealogy/family/member' },
  { en: 'INCOMPLETE', label: '待完善', value: n(summary.value.incompleteCount), to: '/genealogy/family/member' },
  { en: 'QUEUE', label: '待办合计', value: n(summary.value.pendingTotal), to: '/genealogy/family/archive' },
  { en: 'BOOK', label: '谱书页数', value: n(summary.value.bookPages), to: '/portal/book' },
  { en: 'GEN', label: '最大世代', value: n(summary.value.maxGeneration), to: '/genealogy/family/generation' }
])
const health = computed(() => [
  { label: '字辈条目 / 在用', value: `${n(summary.value.generationCount)} / ${n(summary.value.generationInUse)}` },
  ...
  { label: '资助窗口', value: summary.value.scholarshipWindowOpen ? `开放 ${summary.value.scholarshipYear || ''}` : '未开放' },
  { label: '回收站', value: n(summary.value.recycleCount) }
])
const typeLabel = (t: string) => ({ ARCHIVE: '档案', FEED: '动态', SCHOLARSHIP: '资助', COMMENT: '评论' }[t] || t)
const fmt = (t: string) => {
  if (!t) return ''
  const d = new Date(t)
...
.hud {
  ...
  background:
    ...
    radial-gradient(900px 500px at 100% 0%, rgba(78, 205, 196, 0.12), transparent 50%),
    #07090e;
}
.grid-bg {
  pointer-events: none;
...
.scan {
  ...
  position: absolute;
  inset: 0;
  background: linear-gradient(180deg, transparent 0%, rgba(78, 205, 196, 0.08) 50%, transparent 100%);
...
h1 { margin: 0; font-size: 32px; letter-spacing: 6px; font-weight: 700; color: #f4f7f7; }
.meta { margin: 8px 0 0; color: var(--dim); font-size: 13px; }
.hero-right { text-align: right; }
.clock { font-variant-numeric: tabular-nums; color: var(--gold); letter-spacing: 2px; margin-bottom: 10px; }
.portal-btn { background: transparent !important; border: 1px solid var(--cyan) !important; color: var(--cyan) !important; }
.alerts { display: flex; flex-wrap: wrap; gap: 10px; margin-bottom: 22px; }
.pill {
  display: flex;
  align-items: center;
  gap: 10px;
...
.pill.hot { border-color: var(--red); box-shadow: 0 0 12px rgba(196, 92, 74, 0.35); }
.pill-label { color: var(--dim); font-size: 12px; }
.pill-num { font-size: 18px; font-weight: 700; color: var(--cyan); }
.pill.hot .pill-num { color: #ffb4a8; }
...
.kpi-label { font-size: 10px; letter-spacing: 2px; color: var(--cyan); }
.kpi-val { font-size: 30px; font-weight: 700; margin: 6px 0 2px; color: #fff; }
.kpi-name { font-size: 12px; color: var(--dim); }
.panels { display: grid; grid-template-columns: 1.2fr 0.8fr; gap: 14px; margin-bottom: 18px; }
.panel {
  background: var(--panel);
...
.panel-h { font-size: 15px; font-weight: 700; margin-bottom: 12px; display: flex; justify-content: space-between; }
.panel-h span { color: var(--dim); font-size: 11px; letter-spacing: 2px; font-weight: 400; }
.todo {
  width: 100%;
  display: flex;
  align-items: center;
...
.tag { font-size: 11px; color: var(--gold); border: 1px solid rgba(212, 165, 116, 0.4); padding: 1px 6px; }
.todo-title { flex: 1; }
.todo-time, .empty { color: var(--dim); font-size: 12px; }
.health { display: grid; gap: 8px; }
.h-row { display: flex; justify-content: space-between; padding: 6px 0; border-bottom: 1px solid rgba(255,255,255,0.04); color: var(--dim); }
.h-row b { color: var(--ink); font-weight: 600; }
.ritual { margin-top: 16px; padding: 12px; border: 1px solid rgba(196, 92, 74, 0.45); cursor: pointer; }
.ritual-title { font-size: 16px; margin: 4px 0; }
.links { display: grid; grid-template-columns: repeat(6, minmax(0, 1fr)); gap: 10px; }
.hex {
...
.hex:hover { border-color: var(--gold); color: var(--gold); }
.hex-label { font-size: 16px; font-weight: 700; }
.hex-hint { font-size: 11px; color: var(--dim); margin-top: 4px; }
