package chen.genealogy.framework.tenant.config;

import chen.genealogy.framework.common.biz.system.tenant.TenantCommonApi;
import chen.genealogy.framework.common.enums.WebFilterOrderEnum;
import chen.genealogy.framework.mybatis.core.util.MyBatisUtils;
import chen.genealogy.framework.redis.config.ChenCacheProperties;
import chen.genealogy.framework.security.core.service.SecurityFrameworkService;
import chen.genealogy.framework.tenant.core.aop.TenantIgnore;
import chen.genealogy.framework.tenant.core.aop.TenantIgnoreAspect;
import chen.genealogy.framework.tenant.core.db.TenantDatabaseInterceptor;
import chen.genealogy.framework.tenant.core.job.TenantJobAspect;
import chen.genealogy.framework.tenant.core.mq.rabbitmq.TenantRabbitMQInitializer;
import chen.genealogy.framework.tenant.core.mq.redis.TenantRedisMessageInterceptor;
import chen.genealogy.framework.tenant.core.mq.rocketmq.TenantRocketMQInitializer;
import chen.genealogy.framework.tenant.core.redis.TenantRedisCacheManager;
import chen.genealogy.framework.tenant.core.security.TenantSecurityWebFilter;
import chen.genealogy.framework.tenant.core.service.TenantFrameworkService;
import chen.genealogy.framework.tenant.core.service.TenantFrameworkServiceImpl;
import chen.genealogy.framework.tenant.core.web.TenantContextWebFilter;
import chen.genealogy.framework.tenant.core.web.TenantVisitContextInterceptor;
...
@AutoConfiguration
@ConditionalOnProperty(prefix = "chen.tenant", value = "enable", matchIfMissing = true) // 允许使用 chen.tenant.enable=false 禁用多租户
@EnableConfigurationProperties(TenantProperties.class)
public class ChenTenantAutoConfiguration {
    ...
    @ConditionalOnClass(name = "chen.genealogy.framework.mq.redis.core.interceptor.RedisMessageInterceptor")
    ...
    public RedisCacheManager tenantRedisCacheManager(RedisTemplate<String, Object> redisTemplate,
                                                     ...
                                                     ChenCacheProperties chenCacheProperties,
                                                     TenantProperties tenantProperties) {
        // 创建 RedisCacheWriter 对象
        RedisConnectionFactory connectionFactory = Objects.requireNonNull(redisTemplate.getConnectionFactory());
        RedisCacheWriter cacheWriter = RedisCacheWriter.nonLockingRedisCacheWriter(connectionFactory,
                BatchStrategies.scan(chenCacheProperties.getRedisScanBatchSize()));
