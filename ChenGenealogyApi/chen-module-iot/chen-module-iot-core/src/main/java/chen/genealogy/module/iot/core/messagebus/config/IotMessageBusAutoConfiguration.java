package chen.genealogy.module.iot.core.messagebus.config;

import chen.genealogy.framework.mq.redis.core.RedisMQTemplate;
import chen.genealogy.framework.mq.redis.core.job.RedisPendingMessageResendJob;
import chen.genealogy.framework.mq.redis.core.job.RedisStreamMessageCleanupJob;
import chen.genealogy.framework.mq.redis.core.stream.AbstractRedisStreamMessage;
import chen.genealogy.framework.mq.redis.core.stream.AbstractRedisStreamMessageListener;
import chen.genealogy.module.iot.core.messagebus.core.IotMessageBus;
import chen.genealogy.module.iot.core.messagebus.core.kafka.IotKafkaMessageBus;
import chen.genealogy.module.iot.core.messagebus.core.local.IotLocalMessageBus;
import chen.genealogy.module.iot.core.messagebus.core.rabbitmq.IotRabbitMQMessageBus;
import chen.genealogy.module.iot.core.messagebus.core.redis.IotRedisMessageBus;
import chen.genealogy.module.iot.core.messagebus.core.rocketmq.IotRocketMQMessageBus;
...
public class IotMessageBusAutoConfiguration {
    ...
    @Configuration
    @ConditionalOnProperty(prefix = "chen.iot.message-bus", name = "type", havingValue = "local", matchIfMissing = true)
    ...
    @Configuration
    @ConditionalOnProperty(prefix = "chen.iot.message-bus", name = "type", havingValue = "rocketmq")
    ...
    @Configuration
    @ConditionalOnProperty(prefix = "chen.iot.message-bus", name = "type", havingValue = "kafka")
    ...
    /**
     * 特殊：由于 ChenRedisMQConsumerAutoConfiguration 关于 Redis stream 的消费是动态注册，所以这里只能拷贝相关的逻辑！！！
     *
     * @see chen.genealogy.framework.mq.redis.config.ChenRedisMQConsumerAutoConfiguration
     */
    @Configuration
    @ConditionalOnProperty(prefix = "chen.iot.message-bus", name = "type", havingValue = "redis")
    ...
    @Configuration
    @ConditionalOnProperty(prefix = "chen.iot.message-bus", name = "type", havingValue = "rabbitmq")
